...
const data2 = [{ x : 1500, y: 208500 }]


const result = spawn("python", [ "..\\app\\backend\\main.py", "predict", "..\\app\\backend\\snare.wav" ])
...
result.stdout.pipe(split(JSON.parse)).on("data", data => {
  ...
  .enter()
  .append("circle")
    .attr("cx", function (d: any) { return x(1500); } )
    .attr("cy", function (d: any) { return y(d.classes.Snare * 300000); } )
    .attr("r", 7)
    .style("fill", "#69b3a2")
    .style("opacity", 0.3)
    .style("stroke", "white")
